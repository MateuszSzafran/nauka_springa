package pl.mszafran.dynamic_styles;
...
class EmployeeRepository {
    ...
    BigDecimal getAvgSalary(){
        BigDecimal sum = findAll().stream().map(Employee::getSalary).reduce(BigDecimal.ZERO, BigDecimal::add);
        return sum.divide(BigDecimal.valueOf(employees.size()), RoundingMode.HALF_UP);
    }
}
